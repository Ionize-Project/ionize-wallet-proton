...
export default class Balance extends Component<Props, State> {
  ...
  switchCurrency = () => {
    ...
    if (displayCurrency === 'fiat') { */
      ...
      configManager.modifyConfig('displayCurrency', 'INZ');
      eventEmitter.emit('modifyCurrency', 'INZ');
    // }
  ...
  render() {
    ...
    if (displayCurrency === 'INZ') {
      balanceTooltip =
        `Unlocked: ${atomicToHuman(unlockedBalance, true)} ${il8n.INZ}<br>` +
        `Locked: ${atomicToHuman(lockedBalance, true)} ${il8n.INZ}`;
